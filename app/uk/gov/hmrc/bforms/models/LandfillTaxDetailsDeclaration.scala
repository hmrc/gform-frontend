...
object LandfillTaxDetailsDeclaration {
  implicit val formats = Json.format[LandfillTaxDetailsDeclaration]

  val landfillTaxDetailsDeclarationMapping: Mapping[LandfillTaxDetailsDeclaration] = {
    mapping(
    ...
    )(LandfillTaxDetailsDeclaration.apply)(LandfillTaxDetailsDeclaration.unapply)
  }

  def validateEmail(landfillTaxDetailsDeclaration: LandfillTaxDetailsDeclaration): Boolean = {
    landfillTaxDetailsDeclaration.emailAddress match {
      case Some(e) if (e != landfillTaxDetailsDeclaration.confirmEmailAddress.getOrElse("")) => false
      case _ => true
    }
  }

  def validateEnvironmentalTotal(landfillTaxDetailsDeclaration: LandfillTaxDetailsDeclaration): Boolean =
    landfillTaxDetailsDeclaration.taxCreditClaimedForEnvironment == landfillTaxDetailsDeclaration.environmentalBodies.map(_.amount).sum

  val form = Form(landfillTaxDetailsDeclarationMapping
