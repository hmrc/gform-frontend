...
object Permissions {
  ...
  def apply(operation: OperationWithForm, role: Role, status: FormStatus): PermissionResult =
    (operation, role, status) match {
      ...
      case (ReviewReturned, Reviewer, NeedsReview | Returning)             => permitted(operation, role, status)
      ...
      case (ViewSummary, Reviewer, NeedsReview | Returning | Accepted)     => permitted(operation, role, status)
