...
sealed trait TextConstraint {
  val defaultCssClassName: String = this match {
    ...
    case WholeSterling(_, _)          => CssClassSize._10
...
case class WholeSterling(positiveOnly: Boolean, roundingMode: RoundingMode = RoundingMode.defaultRoundingMode)
    extends TextConstraint
...
object TextConstraint {
  ...
  private def getSizeClassForDisplayWidthForNumber(
    ...
    (constraint, displayWidth) match {
      ...
      case (WholeSterling(_, _), DisplayWidth.XS) => CssClassSize._3
  ...
  def getSizeClass(constraint: TextConstraint, displayWidth: DisplayWidth.DisplayWidth): String =
    (constraint, displayWidth) match {
      ...
      case (
            Number(_, _, _, _) | PositiveNumber(_, _, _, _) | Sterling(_, _) | WholeSterling(_, _) |
            UkBankAccountNumber | UkSortCodeFormat,
            displayWidth
          ) =>
...
object SelectionCriteria {
  ...
  implicit val format: OFormat[SelectionCriteria] = derived.oformat()
}

case class SimplifiedSelectionCriteria(column: CsvColumnName, value: List[String])

object SimplifiedSelectionCriteria {
  ...
  def convertToSimplifiedSelectionCriteria[D <: DataOrigin](
    ...
    case SelectionCriteria(c, SelectionCriteriaReference(expr, cName)) =>
      ...
      val reg: Option[Register] = aFormComponents.find(_.id === expr.formComponentId).collectFirst {
        case IsText(Text(Lookup(r, _), _, _, _, _, _, _)) => r
      }
