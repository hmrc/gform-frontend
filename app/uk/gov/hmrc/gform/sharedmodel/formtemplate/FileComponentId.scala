...
import uk.gov.hmrc.gform.sharedmodel.form.{ FileId, FormField }
...
sealed trait FileComponentId {
  ...
  def toModelComponentId(): ModelComponentId = FormComponentId(this.value()).modelComponentId

  def toFormField(value: String): FormField = FormField(toModelComponentId(), value)
...
object FileComponentId {
  ...
  def fromHtmlFieldId(htmlFieldId: HtmlFieldId): FileComponentId = htmlFieldId
    .fold[FileComponentId](pure => FileComponentId.Single(pure.modelComponentId.toFormComponentId))(indexed =>
      FileComponentId.Multi(indexed.formComponentId, indexed.index.toInt)
    )

  def fromFormField(formField: FormField): FileComponentId = fromString(formField.id.toFormComponentId.value)
