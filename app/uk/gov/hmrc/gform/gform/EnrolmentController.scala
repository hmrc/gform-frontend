...
class EnrolmentController(
  ...
  private def toSingleton(enrolmentSection: EnrolmentSection): Singleton =
    Singleton(enrolmentSection.toPage)
          ...
          implicit sse =>
            implicit request =>
              implicit l => {
                ...
                val enrolmentAlreadyLinkedPage = new EnrolmentBlockedPage(formTemplate, changeOrSignoutChoice)
                Ok(
      ...
      _ =>
        enrolmentOutcomes =>
          implicit sse =>
            implicit request =>
              implicit l => {
                val insufficientCredentialsPage = new EnrolmentBlockedPage(formTemplate, changeOrSignoutChoice)
                Ok(
  ...
  private def withEnrolmentSection(
    ...
    auth.asyncGGAuth(formTemplateId) { implicit request: Request[AnyContent] => implicit l => cache =>
      ...
      val sse = smartStringEvaluatorFactory.noForm(initFormEvaluator.evalExpr)
            ...
            implicit request =>
              implicit l => {
                ...
                changeOrSignoutChoice
                  ...
                  .fold(
                    errorForm => {
                      val enrolmentAlreadyLinkedPage = new EnrolmentBlockedPage(formTemplate, errorForm)
                      BadRequest(
                        enrolment_already_linked_page(
                          enrolmentAlreadyLinkedPage,
                          enrolmentOutcomes.alreadyLinkedPage,
                    ...
                    successFormAction => handleChangeOrSignoutSuccess(successFormAction, formTemplate)
                  )
              }
    )
            ...
            implicit request =>
              implicit l =>
                changeOrSignoutChoice
                  ...
                  .fold(
                    errorForm => {
                      val insufficientCredentialsPage = new EnrolmentBlockedPage(formTemplate, errorForm)
                      BadRequest(
                        insufficient_credentials_page(
                          insufficientCredentialsPage,
                          enrolmentOutcomes.insufficientCredentialsPage,
  ...
  private def handleChangeOrSignoutSuccess(formAction: String, formTemplate: FormTemplate) =
    formAction match {
  ...
  def formModelOpticsFromEnrolmentSection(
  ...
  def showEnrolment(formTemplateId: FormTemplateId) =
    auth.asyncGGAuth(formTemplateId) { implicit request: Request[AnyContent] => implicit l => cache =>
      if (cache.retrievals.getCredentialRole.contains(Assistant)) {
            ...
            Future.successful(
              Ok(
                renderEnrolmentSection(
                  ...
                  cache.retrievals,
                  enrolmentSection,
                  formModelOpticsFromEnrolmentSection(enrolmentSection, cache),
                  Nil,
  ...
  private def renderEnrolmentSection(
    ...
    implicit val sse = smartStringEvaluatorFactory(formModelOptics.formModelVisibilityOptics)
    ...
    auth.asyncGGAuth(formTemplateId) { implicit request => implicit l => cache =>
      ...
      formTemplate.authConfig match {
        case HasEnrolmentSection((serviceId, enrolmentSection, postCheck, lfcev, enrolmentOutcomes)) =>
          val formModelOptics: FormModelOptics = formModelOpticsFromEnrolmentSection(enrolmentSection, cache)
          ...
          processResponseDataFromBody(request, formModelOptics) { requestRelatedData => variadicFormData => _ =>
            val formModelOptics = FormModelOptics
              .mkFormModelOptics[SectionSelectorType.EnrolmentOnly](
                variadicFormData,
                cache,
                cache.toCacheData,
                None,
            ...
            def handleContinueWithData(formModelOptics: FormModelOptics) = {
              ...
              implicit val sse = smartStringEvaluatorFactory(formModelVisibilityOptics)
              ...
              val formHandlerResultF: Future[FormHandlerResult] =
                formValidator.validatePageModelBySectionNumber(
                  formModelOptics,
