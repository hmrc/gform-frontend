...
class SectionRenderingService(frontendAppConfig: FrontendAppConfig, lookupRegistry: LookupRegistry)(
  ...
  def renderSection(
    ...
    val snippetsForFields = section.fields.map(fieldValue =>
      htmlFor(fieldValue, formTemplate._id, 0, ei, fieldData, validatedType, fieldValue.onlyShowOnSummary, obligations))
  ...
  def renderDeclarationSection(
    ...
    val snippets = formTemplate.declarationSection.fields.map(fieldValue =>
      htmlFor(fieldValue, formTemplate._id, 0, ei, fieldData, validatedType, obligations = NotChecked))
    val pageLevelErrorHtml = generatePageLevelErrorHtml(listResult, List.empty)
    val renderingInfo = SectionRenderingInformation(
      formTemplate._id,
      maybeAccessCode,
  ...
  private def htmlFor(
    ...
    fieldValue.`type` match {
      ...
      case g @ Group(_, _, _, _, _, _) =>
        htmlForGroup(g, formTemplateId, fieldValue, index, ei, data, maybeValidated, obligations)
      ...
      case t @ Text(_, _, _, _) => renderText(messages, l)(t, fieldValue, index, maybeValidated, ei, data, isHidden)
      case t @ TextArea(_, _, _) =>
        renderTextArea(messages, l)(t, fieldValue, index, maybeValidated, ei, data, isHidden)
      case Choice(choice, options, orientation, selections, optionalHelpText) =>
        htmlForChoice(
          ...
          data)
      case FileUpload() =>
        htmlForFileUpload(fieldValue, formTemplateId, index, ei, data, ei.retrievals, maybeValidated)
      ...
      fieldValue.presentationHint match {
        case Some(xs) if xs.contains(TotalValue) =>
          asTotalValue(
            ...
            ei.formLevelHeading)
        case _ =>
          asStandard(fieldValue, t, prepopValue, validatedValue, index, ei.section.title.value(l), ei.formLevelHeading)
      }
    }
  }
  ...
  private def htmlForGroup(
    ...
    validatedType: ValidatedType[ValidationResult],
    obligations: Obligations)(implicit request: Request[_], messages: Messages, l: LangADT): Html = {
    val grpHtml = htmlForGroup0(grp, formTemplateId, fieldValue, index, ei, data, validatedType, obligations)
    ...
    fieldValue.presentationHint match {
      case Some(list) if list.contains(CollapseGroupUnderLabel) =>
        html.form.snippets.collapsable(fieldValue.id, fieldValue.label.value(l), grpHtml, isChecked)
      case _ => grpHtml
    }
  }
  ...
  private def htmlForGroup0(
    ...
    val maybeHint =
      fieldValue.helpText.map(localisedString => localisedString.value(l)).map(markDownParser).map(Html.apply)
