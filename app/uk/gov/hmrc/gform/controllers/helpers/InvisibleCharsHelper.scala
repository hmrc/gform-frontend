...
object InvisibleCharsHelper {
  ...
  private val INVISIBLE_CHAR_MAP: Map[Char, UnicodeDescMapping] =
    INVISIBLE_UNICODE_DESC_MAPPING.groupBy(_.code).mapValues(_.head).toMap
  ...
  def invisibleCharMatches(input: String): Map[Char, Int] =
    input.toCharArray.filter(INVISIBLE_CHAR_MAP.contains).groupBy(identity).mapValues(_.length).toMap
