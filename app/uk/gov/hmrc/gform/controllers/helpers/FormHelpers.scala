...
import uk.gov.hmrc.gform.models.form.{FormData, FormId}
...
object FormHelpers {
  ...
  def processResponseDataFromBody(request: RequestWithTemplate[AnyContent])
                                 (continuation: Map[FieldId, Seq[String]] => Future[Result])
