...
object ValidationUtil {
  ...
  def evaluateWithSuffix(fieldValue: FormComponent, gformErrors: Map[FormComponentId, Set[String]])(dGetter: (FormComponentId) => Seq[String]): List[(FormComponentId, FormFieldValidationResult)] = {
  ...
  def evaluateWithoutSuffix(fieldValue: FormComponent, gformErrors: Map[FormComponentId, Set[String]])(dGetter: (FormComponentId) => Seq[String]): (FormComponentId, FormFieldValidationResult) = {
  ...
  def evaluateValidationResult(atomicFields: List[FormComponent], validationResult: ValidatedType, data: Map[FormComponentId, Seq[String]],
    ...
    val resultErrors: List[FormFieldValidationResult] = atomicFields.map { fieldValue =>
      ...
      fieldValue.`type` match {
        case sortCode @ UkSortCode(_) =>
          val valSuffixResult: List[(FormComponentId, FormFieldValidationResult)] = evaluateWithSuffix(fieldValue, gFormErrors)(dataGetter)
          val valWithoutSuffixResult: (FormComponentId, FormFieldValidationResult) = evaluateWithoutSuffix(fieldValue, gFormErrors)(dataGetter)
  ...
  def validateFileUploadHasScannedFiles(fieldValues: List[FormComponent], e: Envelope): Validated[GformError, Unit] = {
    ...
    def errors(fieldValue: FormComponent, defaultErr: String): Set[String] = Set(fieldValue.errorMessage.getOrElse(defaultErr))
