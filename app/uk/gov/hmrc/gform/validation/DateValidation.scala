...
class DateValidation(implicit messages: Messages, l: LangADT) {
    ...
    date.constraintType match {
      ...
      case AnyDate =>
        validateInputDate(
          fieldValue,
          fieldValue.id,
          fieldValue.errorMessage.map(ls => ls.value(l)),
          data,
          otherFieldValue).andThen(lDate => validationSuccess)
  ...
  private def validateConcreteDateWithMessages(
    ...
    validateInputDate(fieldValue, fieldValue.id, fieldValue.errorMessage.map(ls => ls.value(l)), data).andThen(
      inputDate =>
  ...
  private def validateTodayWithMessages(
    ...
    offset: OffsetDate,
    data: FormDataRecalculated): Validated[GformError, Unit] =
    validateInputDate(fieldValue, fieldValue.id, fieldValue.errorMessage.map(ls => ls.value(l)), data)
      .andThen(
        inputDate =>
          validateToday(
            fieldValue,
