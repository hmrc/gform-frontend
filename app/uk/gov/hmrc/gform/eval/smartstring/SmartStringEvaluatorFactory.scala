  ...
  def apply(
    ...
    new SmartStringEvaluator {
      ...
      override def evalEnglish(s: SmartString, markDown: Boolean): String = {
        ...
        withMessages(s, markDown, englishImplicitMessages, englishImplicitLangADT)
      }

      def withMessages(s: SmartString, markDown: Boolean, messages: Messages, l: LangADT): String = {

        def formatExpr(expr: Expr, markDown: Boolean): String = {
        ...
        def evalChoice(fcId: FormComponentId, typeInfo: TypeInfo, markDown: Boolean) =
          formModelVisibilityOptics.formModel.fcLookup
            .get(fcId)
            .collect {
        ...
        def stringRepresentation(typeInfo: TypeInfo): String =
        ...
        def mapChoiceSelectedIndexes(typeInfo: TypeInfo, f: Seq[String] => String): String =
          formModelVisibilityOptics
            .evalAndApplyTypeInfo(typeInfo)
            .optionRepresentation
            .fold(stringRepresentation(typeInfo))(f(_))

        def govukListRepresentation(typeInfo: TypeInfo, isBulleted: Boolean, markDown: Boolean): String = {
