...
sealed trait ExpressionResult extends Product with Serializable {
  ...
  def +(er: ExpressionResult)(implicit messages: Messages): ExpressionResult = er match {
    ...
    case t: NumberResult =>
      fold[ExpressionResult](identity)(_ => t)(_ => t)(_ + t)(s => StringResult(s.value + t.value.toString))(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(l => ListResult(l.list.map(_ + t)))
    case t: StringResult =>
      fold[ExpressionResult](identity)(_ => t)(_ => t)(n => StringResult(n.value.toString + t.value))(_ + t)(invalidAdd)(d => StringResult(d.asString + t.value))(invalidAdd)(invalidAdd)(p => StringResult(p.asString + t.value))(invalidAdd)
    case t: OptionResult  => Invalid(s"Unsupported operation, cannot add OptionResult and $t")
    case t: DateResult    =>
      fold[ExpressionResult](identity)(_ => t)(_ => t)(invalidAdd)(_ + StringResult(t.asString))(invalidAdd)(d => StringResult(d.asString + t.asString))(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)
    case t: TaxPeriodResult => Invalid(s"Unsupported operation, cannot add TaxPeriodResult and $t")
    case t: PeriodResult  =>
      fold[ExpressionResult](identity)(_ => t)(_ => t)(invalidAdd)(_ + StringResult(t.asString))(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(_ + t)(invalidAdd)
    case t: AddressResult => Invalid(s"Unsupported operation, cannot add AddressResult and $t")
    case t: ListResult => fold[ExpressionResult](identity)(_ => t)(identity)(e => ListResult(t.list.map(_ + e)))(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(invalidAdd)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a + b }))
    // format: on
  }
  ...
  def -(er: ExpressionResult): ExpressionResult = er match {
    ...
    case t: Hidden.type  => this
    case t: Empty.type   => this
    case t: NumberResult => fold[ExpressionResult](identity)(_ => t)(identity)(_ - t)(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(l => ListResult(l.list.map(_ - t)))
    ...
    case t: TaxPeriodResult => Invalid("Unsupported operation, cannot substract TaxPeriodResult")
    ...
    case t: ListResult => fold[ExpressionResult](identity)(_ => t)(identity)(e => ListResult(t.list.map(e - _)))(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(invalidSubstract)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a - b }))
    // format: on
  }
  ...
  def *(er: ExpressionResult): ExpressionResult = er match {
    // format: off
    case t: Invalid      => t
    case t: Hidden.type  => fold[ExpressionResult](identity)(identity)(identity)(_ => NumberResult(0))(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(l => ListResult(l.list.map(_ => NumberResult(0))))
    case t: Empty.type   => this
    case t: NumberResult => fold[ExpressionResult](identity)(_ => NumberResult(0))(identity)(_ * t)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(l => ListResult(l.list.map(_ * t)))
    ...
    case t: TaxPeriodResult => Invalid("Unsupported operation, cannot multiply TaxPeriodResult")
    ...
    case t: ListResult => fold[ExpressionResult](identity)(_ => ListResult(t.list.map(_ => NumberResult(0))))(identity)(e => ListResult(t.list.map(_ * e)))(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(invalidMult)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a * b }))
    // format: on
  }
  ...
  def /(er: ExpressionResult): ExpressionResult = er match {
    // format: off
    case t: Invalid      => t
    case t: Hidden.type  => fold[ExpressionResult](identity)(identity)(identity)(_ => NumberResult(0))(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(l => ListResult(l.list.map(_ => NumberResult(0))))
    case t: Empty.type   => this
    case t: NumberResult => fold[ExpressionResult](identity)(_ => NumberResult(0))(identity)(_ / t)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(l => ListResult(l.list.map( _ / t)))
    ...
    case t: TaxPeriodResult => Invalid("Unsupported operation, cannot divide TaxPeriodResult")
    ...
    case t: ListResult => fold[ExpressionResult](identity)(_ => ListResult(t.list.map(_ => NumberResult(0))))(identity)(e => ListResult(t.list.map(e / _)))(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(invalidDivide)(l => ListResult(l.list.zip(t.list).map{case (a,b) => a / b }))
    // format: on
  }
  ...
  def before(er: ExpressionResult): Boolean = this match {
    ...
    case t: TaxPeriodResult => false // TaxPeriodResult is turned to DateResult
    case t: PeriodResult    => false
    case t: AddressResult   => false
  ...
  private def ifPeriodResult(f: Period => Boolean): Boolean =
    fold[Boolean](_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ =>
      false
    )(r => f(r.value))(_ => false)
  private def ifListResult(f: List[ExpressionResult] => Boolean): Boolean =
    fold[Boolean](_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(_ => false)(r => false)(_ =>
      false
    )(_ => false)(r => f(r.list))
  ...
  def fold[B](
    ...
    case t: TaxPeriodResult => h(t)
    case t: AddressResult   => i(t)
    case t: PeriodResult    => j(t)
    case t: ListResult      => k(t)
  }
}
...
object ExpressionResult {
  ...
  case class TaxPeriodResult(month: java.time.Month, year: Int) extends ExpressionResult {
    def asString(implicit messages: Messages) = {
      val monthStr = messages(s"date.${month.getDisplayName(TextStyle.FULL, Locale.UK)}")
      s"$monthStr $year"
    }
  }
