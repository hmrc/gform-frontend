...
object AllFormComponentExpressions extends ExprExtractorHelpers {
  def unapply(fc: FormComponent): Option[List[ExprMetadata]] = {
    ...
    def fromRcElements(revealingChoiceElements: List[RevealingChoiceElement]): List[ExprMetadata] =
      revealingChoiceElements.flatMap { case RevealingChoiceElement(choice, _, _) =>
    ...
    val componentTypeExprs: List[ExprMetadata] = fc match {
      case IsText(Text(Lookup(_, sc), _, _, _, _, _, _, _, _)) => fcLookupExpr(sc)
      ...
      case IsChoice(Choice(_, options, _, _, optionHelpText, _, _, _, _, _, _)) =>
        toPlainExprs(
          ...
          options.toList.flatMap(_.hint.toList.flatMap(_.allInterpolations)),
      ...
      case IsMiniSummaryList(MiniSummaryList(rows, _, _)) =>
        toPlainExprs(
          (rows.collect { case MiniSummaryRow.ValueRow(Some(key), _, _, _, _) => key.allInterpolations }).flatten,
          (rows.collect { case MiniSummaryRow.SmartStringRow(Some(key), v, _, _, _) =>
            key.allInterpolations ++ v.allInterpolations
          }).flatten,
          rows.collect { case MiniSummaryRow.ValueRow(_, MiniSummaryListValue.AnyExpr(e), _, _, _) => e },
          rows.collect { case MiniSummaryRow.ValueRow(_, MiniSummaryListValue.Reference(e), _, _, _) => e }
        )
