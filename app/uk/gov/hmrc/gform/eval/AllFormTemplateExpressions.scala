...
object AllFormTemplateExpressions extends ExprExtractorHelpers {
  def apply(formTemplate: FormTemplate): Set[ExprMetadata] = {
    ...
    val summarySectionExprs: List[Expr] = {
      val SummarySection(title, _, header, footer, continueLabel, _, _, _, _, _, _, _, _) = formTemplate.summarySection
    ...
    def fromDestinationList(destinationList: DestinationList): List[ExprMetadata] = {
      ...
      def fromDestination(destination: Destination): List[Expr] = destination match {
        ...
        case d: Destination.PegaApi                => d.caseId :: Nil
        case d: Destination.NiRefundClaimApi =>
          List(
            Some(d.bankAccountName),
            Some(d.sortCode),
            Some(d.accountNumber),
            d.rollNumber,
