...
object HasDigits {
  def unapply(expr: ComponentType): Option[MaxDigitsAndRoundingMode] =
    expr match {
      case Text(Number(_, digits, rm, _), _, _, _, _, _, _, _, _)         => Some(MaxDigitsAndRoundingMode(digits, rm))
      case Text(PositiveNumber(_, digits, rm, _), _, _, _, _, _, _, _, _) => Some(MaxDigitsAndRoundingMode(digits, rm))
...
object HasSterling {
  def unapply(expr: ComponentType): Option[MaxDigitsAndRoundingMode] =
    expr match {
      case Text(s: Sterling, _, _, _, _, _, _, _, _) => Some(MaxDigitsAndRoundingMode(2, s.roundingMode))
      case TextArea(s: Sterling, _, _, _, _, _)      => Some(MaxDigitsAndRoundingMode(2, s.roundingMode))
      case Text(s: WholeSterling, _, _, _, _, _, _, _, _) =>
