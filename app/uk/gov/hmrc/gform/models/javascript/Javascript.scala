...
object Javascript {
  ...
  private def toJavascriptFn(
    ...
    def getRoundingMode(fc: FormComponent) =
      fc.`type` match {
        case Text(Number(_, _, rm, _), _, _, _, _, _, _)          => Some(rm)
        case TextArea(Number(_, _, rm, _), _, _, _, _, _)         => Some(rm)
        case Text(PositiveNumber(_, _, rm, _), _, _, _, _, _, _)  => Some(rm)
        case TextArea(PositiveNumber(_, _, rm, _), _, _, _, _, _) => Some(rm)
        case Text(s: Sterling, _, _, _, _, _, _)                  => Some(s.roundingMode)
