...
package object ops {
  ...
  implicit class FormComponentOps(formComponent: FormComponent) {
    ...
    def isUkVrn = formComponent.`type` match {
      case Text(UkVrn, _, _, _, _, _, _, _) => true
      case _                                => false
    }

    def isEORI = formComponent.`type` match {
      case Text(EORI, _, _, _, _, _, _, _) => true
      case _                               => false
    }

    def isUkEORI = formComponent.`type` match {
      case Text(UkEORI, _, _, _, _, _, _, _) => true
      case _                                 => false
    }

    def isNino = formComponent.`type` match {
      case Text(NINO, _, _, _, _, _, _, _) => true
      case _                               => false
    }

    def isCtUtr = formComponent.`type` match {
      case Text(CtUTR, _, _, _, _, _, _, _) => true
      case _                                => false
    }

    def isSaUtr = formComponent.`type` match {
      case Text(SaUTR, _, _, _, _, _, _, _) => true
      case _                                => false
    }

    def isChildBenefitNumber = formComponent.`type` match {
      case Text(ChildBenefitNumber, _, _, _, _, _, _, _) => true
      case _                                             => false
    }

    def isCompanyRegistrationNumber = formComponent.`type` match {
      case Text(CompanyRegistrationNumber, _, _, _, _, _, _, _) => true
      case _                                                    => false
    }
