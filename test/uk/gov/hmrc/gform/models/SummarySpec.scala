...
class SummarySpec extends Spec {
  ...
  "Summary" should "display the summary sections" in new Test {
    val render = SummaryRenderingService
      .summaryRowsForRender(f, formDataRecalculated, Some(accessCode), formTemplate, envelope, NotChecked)
    render.size should be(6)
  ...
  it should "display links to page sections" in new Test {
    ...
    val render =
      SummaryRenderingService
        .summaryRowsForRender(f, FormDataRecalculated.empty, Some(accessCode), formTemplate, envelope, NotChecked)
  ...
  it should "display the title when shortName is not present in the section" in new Test {
    ...
    doc.getElementsByClass("govuk-heading-m").text().toLowerCase should include("your details")
  }
  ...
  it should "display the shortName as section title if present" in new Test {
    ...
    val doc = Jsoup.parse(render.head.toString())
    doc.getElementsByClass("govuk-heading-m").text().toUpperCase should include(shortName)
  }
  ...
  it should "not render sections with includeIf expressions that evaluate to false" in new Test {
    ...
    val renderWithDataMatching = SummaryRenderingService.summaryRowsForRender(
      f,
    ...
    renderWithDataMatching.size shouldBe 2
    val renderWithDataMismatch = SummaryRenderingService.summaryRowsForRender(
      f,
...
//
  ...
  "The Change hrefs" should "link to the correct page" in new Test {
    ...
    val summaryForRender = SummaryRenderingService.summaryRowsForRender(
      f,
      FormDataRecalculated(
        Set(IncludeIfGN(FormComponentId("includeId_X"), includeIf)),
        RecData.fromData(VariadicFormData.ones(FormComponentId("firstName") -> "*Not*Pete"))),
    ...
    {
      val htmlAheadOfSection2 = summaryForRender(3)
