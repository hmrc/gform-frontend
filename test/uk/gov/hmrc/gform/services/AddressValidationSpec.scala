...
class AddressValidationSpec extends FlatSpec with Matchers with EitherMatchers with ScalaFutures {
  ...
  "Address validation" should "throw back custom validation" in {
    ...
    val speccedAddress = FieldValue(FieldId("x"), address, "l", None, None, true, true, false, Some("New Error Message"))
  ...
  "Address validation" should "pass with valid data" in {
    ...
    val data = Map(
      FieldId("x-uk") -> Seq("true"),
      FieldId("x-street1") -> Seq(List.fill(35)("a").mkString),
      FieldId("x-street2") -> Seq(List.fill(35)("a").mkString),
      FieldId("x-street3") -> Seq(List.fill(35)("a").mkString),
      FieldId("x-street4") -> Seq(List.fill(27)("a").mkString),
  ...
  "Address validation" should "fail when the field is fails validation" in {
    ...
    result.toEither should beLeft(
      Map(
        FieldId("x-street2") -> Set("this field is too long must be at most 35"),
        FieldId("x-street1") -> Set("this field is too long must be at most 35"),
        FieldId("x-street3") -> Set("this field is too long must be at most 35"),
        FieldId("x-street4") -> Set("this field is too long must be at most 28")
      )
    )
  }
