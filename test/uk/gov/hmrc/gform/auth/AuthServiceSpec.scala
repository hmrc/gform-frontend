...
class AuthServiceSpec extends ExampleData with SpecWithFakeApp {
  ...
  it should "authorise a gg authentication only user when no agentAccess config" in {
    val result =
      authService
        .authenticateAndAuthorise(formTemplate, requestUri, getAffinityGroup, ggAuthorisedSuccessful, None, false)
  ...
  it should "redirect a gg authentication only user when no agentAccess config" in {
    val result =
      authService
        .authenticateAndAuthorise(formTemplateEeitt, requestUri, getAffinityGroup, ggAuthorisedRedirect, None, false)
    result.futureValue should be(AuthRedirect(""))
  }
  ...
  it should "authorise an eeitt authorised user when user is eeitt enrolled" in {
    val result =
      authService
        .authenticateAndAuthorise(formTemplateEeitt, requestUri, getAffinityGroup, ggAuthorisedSuccessful, None, false)
    result.futureValue should be(AuthSuccessful(materialisedRetrievals))
  }
  ...
  it should "not authorise an Ofsted user when they have not been successfully authenticated by the AWS ALB" in {
    val result =
      authService
        .authenticateAndAuthorise(formTemplateAWSALB, requestUri, getAffinityGroup, ggAuthorisedSuccessful, None, true)
  ...
  it should "authorise an Ofsted user when they have been successfully authenticated by the AWS ALB" in {
    ...
    val result =
      authService
        .authenticateAndAuthorise(formTemplateAWSALB, requestUri, getAffinityGroup, ggAuthorisedSuccessful, None, false)
    result.futureValue should be(AuthSuccessful(materialisedRetrievalsOfsted))
  }

  it should "not authorise an non-admin Ofsted user when the user accesses an admin only endpoint" in {
    val jwt =
      "..7mBYP9FKoIPf7JBZ9qMcm90n1UNICrWyCBadi5xcjs0pKGGWFlDZLVKmvHSmpCK731JCrz49VPwDQYfsY_I_UA=="
    implicit val hc: HeaderCarrier = HeaderCarrier(otherHeaders = Seq(("X-Amzn-Oidc-Data" -> jwt)))
    val result =
