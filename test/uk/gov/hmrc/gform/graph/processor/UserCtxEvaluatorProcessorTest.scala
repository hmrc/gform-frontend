...
class UserCtxEvaluatorProcessorTest extends Spec {
  ...
  forAll(userCtxTable) { (enr, field, serviceId, authConfig, expected) =>
    it should s"evaluate a user ctx with $enr, $field, $serviceId and $authConfig return $expected" in new UserCtxEvaluatorProcessor[Id] {
  ...
  it should "return an empty string when authModule in authConfig is set anonymous" in new UserCtxEvaluatorProcessor[Id] {
    processEvaluation(AnonymousRetrievals(SessionId("id")), UserCtx(EnrolledIdentifier), authConfig) should be(NonConvertible(""))
  }

  lazy val userCtxTable = Table(
    ("enr", "userField", "serviceId", "authConfig", "NoncovertibleResult"),
    (irctEnrolment, EnrolledIdentifier, ServiceId("IR-CT"), HmrcEnrolmentModule(EnrolmentAuth(ServiceId("IR-CT"), Never)), NonConvertible("CT value")),
    (irsaEnrolment, EnrolledIdentifier, ServiceId("IR-SA"), HmrcEnrolmentModule(EnrolmentAuth(ServiceId("IR-SA"), Never)), NonConvertible("SA value")),
    (irsaEnrolment, EnrolledIdentifier, ServiceId("IR-SA"), HmrcAgentWithEnrolmentModule(AllowAnyAgentAffinityUser,EnrolmentAuth(ServiceId("IR-SA"), Never)), NonConvertible("SA value")),
    (irctEnrolment, EnrolledIdentifier, ServiceId(""), HmrcEnrolmentModule(EnrolmentAuth(ServiceId(""), Never)), NonConvertible("")),
    (irctEnrolment, FTAffinityGroup, ServiceId(""), HmrcSimpleModule, NonConvertible("agent")))

  lazy val materialisedRetrievalsAgent = AuthenticatedRetrievals(OneTimeLogin, Enrolments(Set(irsaEnrolment)), Some(CoreAffinityGroup.Agent), None, None, userDetails, None, None)
