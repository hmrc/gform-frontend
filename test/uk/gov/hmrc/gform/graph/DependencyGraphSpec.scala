...
class DependencyGraphSpec extends AnyFlatSpecLike with Matchers with FormModelSupport with VariadicFormDataSupport {
  ...
  val choicePropTable = {
    ...
    Table(
      ("prop", "propSetter"),
      ("options", (choice: Choice) => choice.copy(options = List(OptionData.IndexBased(stringExpr, None)))),
  ...
  val addToListSectionExpressionTable = {
    ...
    Table(
      ...
      ("addAnotherQuestion.options",        expect2, emptyAddToList.copy(addAnotherQuestion = mkFormComponent("choice", emptyChoice.copy(options = List(OptionData.IndexBased(stringExpr, None)))))),
  ...
  val formTemplateExpressionTable = {
    ...
    val ctx = FormCtx("a")
    val stringExpr = toSmartStringExpression("", ctx)
    val emptySummarySection = SummarySection(emptySS, emptySS, emptySS, None, None)
    ...
    val emptyDeclarationSection = DeclarationSection(emptySS, None, None, None, None, List.empty[FormComponent])
